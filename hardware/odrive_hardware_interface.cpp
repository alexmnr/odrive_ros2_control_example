...
namespace odrive_hardware_interface
{
  ...
  hardware_interface::CallbackReturn ODriveHardwareInterface::on_init(const hardware_interface::HardwareComponentInterfaceParams & params) {
    ...
    for (const hardware_interface::ComponentInfo & joint_info : info_.joints)
    {
      ...
      if (joint_info.state_interfaces.size() != 3) {
        RCLCPP_FATAL(rclcpp::get_logger("ODriveHardwareInterface"), "[PARAM] Joint '%s' has %zu state interfaces. 3 expected.", joint_info.name.c_str(), joint_info.state_interfaces.size());
        return hardware_interface::CallbackReturn::ERROR;
      }
  ...
  hardware_interface::CallbackReturn ODriveHardwareInterface::on_configure(const rclcpp_lifecycle::State &) {
    // connect to CAN
    if (!can_intf_.init(can_interface_name_, &event_loop_, std::bind(&ODriveHardwareInterface::on_can_msg, this, _1))) {
      RCLCPP_ERROR(rclcpp::get_logger("ODriveHardwareInterface"), "[CONFIG] Failed to initialize SocketCAN on %s", can_interface_name_.c_str());
      RCLCPP_ERROR(rclcpp::get_logger("ODriveHardwareInterface"), "[CONFIG] Have you run 'sudo ip link set up can0 type can bitrate 1000000'?");
      return CallbackReturn::ERROR;
    }
    RCLCPP_INFO(rclcpp::get_logger("ODriveHardwareInterface"), "[CONFIG] Succesfully initialized SocketCAN on %s", can_interface_name_.c_str());
    ...
    for (int i = 0; i <= 30; i++) {
      ...
      for (auto& joint : joints) {
        if (joint.hw_version.length() != 0 && joint.fw_version.length() != 0) {
          RCLCPP_INFO(rclcpp::get_logger("ODriveHardwareInterface"), "[CONFIG] Found: Joint '%s' can-id '%d' with fw-version '%s' and hw-version '%s'", joint.name.c_str(), joint.can_id, joint.fw_version.c_str(), joint.hw_version.c_str());
          if (joint.fw_version != "0:6:11" || joint.hw_version != "5:2:0") {
            RCLCPP_ERROR(rclcpp::get_logger("ODriveHardwareInterface"), "[CONFIG] Joint '%s' with can-id '%d' is running the wrong version! Please update to 0:6:11 and 5:2:0.", joint.name.c_str(), joint.can_id);
      ...
      if (i == 30) {
        RCLCPP_ERROR(rclcpp::get_logger("ODriveHardwareInterface"), "[CONFIG] Could not reach all motors! Check CAN connection!");
        return CallbackReturn::FAILURE;
      }
    }
    ...
    while (1) {
      ...
      for (auto &joint : joints) {
        if (joint.odrive_error == 0 && joint.position_state != 0) {
          ...
          joint.ready = true;
          RCLCPP_INFO(rclcpp::get_logger("ODriveHardwareInterface"), "[ACTIVATION] Motor with can-id '%d' for joint '%s' is ready!", joint.can_id, joint.name.c_str());
        } else {
      ...
      if (finish) {
        RCLCPP_INFO(rclcpp::get_logger("ODriveHardwareInterface"), "[ACTIVATION] Hardware succesfully activated!");
        break;
        // return hardware_interface::CallbackReturn::SUCCESS;
      }
      rclcpp::sleep_for(std::chrono::milliseconds(500));
      RCLCPP_INFO(rclcpp::get_logger("ODriveHardwareInterface"), "[ACTIVATION] Hardware not ready yet...");
    }
  ...
  void ODriveHardwareInterface::Joint::on_encoder_feedback(const can_frame& frame) {
  ...
  void ODriveHardwareInterface::Joint::on_torque_feedback(const can_frame& frame) {
    ...
    msg.decode_buf(frame.data);
    effort_target = msg.Torque_Target * 100;
    effort_state = msg.Torque_Estimate * 100;
  }
  void ODriveHardwareInterface::Joint::on_heartbeat(const can_frame& frame) {
